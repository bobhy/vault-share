...
This project is "Vault Share".  Prior work focused on the *mechanism* (syncing), but this one is focused on the user benefit, sharing a vault (across users or devices).  

## Vision
Primary goal is to allow users to *share* an Obsidian vault across multiple devices and multiple users.  
All local vaults sync to the same cloud folder structure, and are eventual replicas of one another.
A secondary goal is to provide a backup of the vault in the cloud.
## Components
### Authorization
